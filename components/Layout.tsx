      ...
      >
        ...
        <div className="flex items-center h-20 px-5 shrink-0 overflow-hidden">
          <motion.div 
            ...
            className="size-10 rounded-xl bg-primary flex items-center justify-center text-white shadow-lg shadow-primary/20 shrink-0 cursor-pointer"
          >
        ...
        {/* Navigation */}
        <nav className="flex-1 px-3 space-y-1.5 mt-4 overflow-hidden">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center h-11 px-[10px] rounded-xl transition-colors group relative ${
                isActive(item.path)
            ...
            >
              ...
              {isCollapsed && (
                 <div className="absolute left-full ml-4 px-3 py-2 bg-slate-900 text-white text-[11px] font-black rounded-lg opacity-0 group-hover:opacity-100 transition-all pointer-events-none whitespace-nowrap z-[100] shadow-2xl translate-x-2 group-hover:translate-x-0 border border-white/10">
                    {item.name}
                 </div>
              )}
            </Link>
        ...
        <div className="p-3 border-t border-white/5 mt-auto">
          <div className="flex items-center h-14 px-1 rounded-2xl hover:bg-white/5 transition-all cursor-pointer group overflow-hidden">
            <div className="size-10 rounded-full border border-white/10 p-0.5 overflow-hidden shrink-0 shadow-sm">
